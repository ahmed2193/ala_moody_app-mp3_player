...
import '../../features/Playlists/presentation/cubits/create_edit_playlist/playlists_state.dart';
...
// Constants
const _bottomSheetRadius = Radius.circular(20);
...
void showEditCreatePlaylistBottomSheet(
  ...
  showModalBottomSheet(
    ...
    shape: const RoundedRectangleBorder(
      borderRadius: BorderRadius.vertical(top: _bottomSheetRadius),
    ),
...
class _PlaylistFormState extends State<PlaylistForm> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<EditPlaylistCubit, PlaylistState>(
      listener: (context, state) {
        if (state is PlaylistSuccess) Navigator.pop(context);
      },
  ...
  Widget _buildFormContent(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.only(
        bottom: max(0, MediaQuery.of(context).viewInsets.bottom - 55),
      ),
      child: BlocBuilder<MainCubit, MainState>(
        builder: (context, state) {
          final bool isDarkMode = MainCubit.isDark;
          return Container(
            decoration: Constants.customBackgroundDecoration(isDarkMode: isDarkMode),
  ...
  Widget _buildNameField(BuildContext context) => SupportTextFormField(
        hintText: AppLocalizations.of(context)!
            .translate('give_your_playlist_a_name')!,
        controller: _nameController,
        icon: Icons.list_sharp,
        title: AppLocalizations.of(context)!.translate('playlist_name')!,
        validator: (value) => value != null && value.length >= _minNameLength
            ? null

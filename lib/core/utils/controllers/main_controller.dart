...
class AudioService {
  static final AudioService _instance = AudioService._internal();
  static bool _isPlayerInitialized = false; // لمنع إعادة تهيئة المشغل
  ...
  AudioService._internal() {
    if (!_isPlayerInitialized) {
      ...
      log('AudioPlayer instance created and initialized by AudioService.', name: 'AudioService');
    }
  }

  // دالة للتخلص من المشغل عند عدم الحاجة إليه (مثلاً عند إغلاق التطبيق)
  Future<void> disposePlayer() async {
    if (_isPlayerInitialized) {
      await player.dispose();
      _isPlayerInitialized = false;
      log('AudioPlayer instance disposed by AudioService.', name: 'AudioService');
    }
  }
}

// فئة لمعالجة أخطاء الصوت بشكل مركزي
class AudioErrorHandler {
  static Future<void> handleError(
    Object error,
    StackTrace stackTrace,
    AudioPlayer player, {
    String? context, // سياق إضافي لتسجيل أفضل للأخطاء
  }) async {
    final String contextMessage = context != null ? 'Context: $context' : '';
    log('Audio Error: $error. $contextMessage', stackTrace: stackTrace, name: 'AudioErrorHandler');

  
    // await ErrorReportingService.report(error, stackTrace, context: context);

    
    // UIService.showErrorSnackbar("حدث خطأ أثناء تشغيل الصوت. يرجى المحاولة مرة أخرى.");

    try {
      // محاولة إيقاف المشغل بأمان إذا كان يعمل
      if (player.playing || player.processingState != ProcessingState.idle) {
        await player.stop();
        log('Player stopped due to error.', name: 'AudioErrorHandler');
      }
    } catch (stopError, stopStackTrace) {
      log('Error stopping player after another error: $stopError', stackTrace: stopStackTrace, name: 'AudioErrorHandler');
    }
  }
}
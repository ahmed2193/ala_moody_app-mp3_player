import 'package:alamoody/core/api/base_response.dart';
import 'package:alamoody/core/error/failures.dart';
import 'package:alamoody/core/utils/usecases/usecase.dart';
import 'package:alamoody/features/membership/domain/repositories/plan_details_repository.dart';
import 'package:dartz/dartz.dart';
import 'package:equatable/equatable.dart';

class GetPlanUseCase extends UseCase<BaseResponse, GetPlanParams> {
  final BasePlanRepository basePlanRepository;

  GetPlanUseCase({required this.basePlanRepository});

  @override
  Future<Either<Failure, BaseResponse>> call(GetPlanParams params) async {
    return basePlanRepository.getPlanDetails(params: params);
  }
}

class GetPlanParams extends Equatable {
  final String accessToken;

  const GetPlanParams({
    required this.accessToken,
  });

  @override
  List<Object> get props => [accessToken];
}
